...
const sheetVariants = cva(
  "fixed z-40 gap-4 bg-background lg:rounded-l-2xl md:rounded-l-2xl lg:p-6 md:p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
    ...
    >
      <SheetPrimitive.Close
        className={cn(
          ...
          side === "right"
            ? "right-4 md:right-[calc(672px+5px)] lg:right-[calc(666px+12px)]"
            : "left-4"
        )}
      >
        <Cross2Icon className="h-6 w-6 text-white [transition:transform_200ms_ease-in-out] group-hover:[transform:rotate(90deg)]" />
